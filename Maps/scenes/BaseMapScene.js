class BaseMapScene extends Phaser.Scene {
    ...
    create() {
        ...
        .on('pointerdown', () => {
            ...
            const cfg = window.ReadingState.mapConfig[mapKey];
            const storageKey = cfg.storage;
            const continentProg = window.ReadingState[storageKey] || 0;

            // ⭐⭐⭐ 大陆已完成 → 禁用按钮（不会破坏 pointer 状态） ⭐⭐⭐
    ...
    showBookList() {
        const mapKey = this.scene.key;
        const mapCfg = window.ReadingState.mapConfig[mapKey];
        const globalBooks = window.ReadingState.globalBooks;
        const completedBookIds = window.ReadingState.completedBookIds || {};
        const mapSelectedBook = window.ReadingState.mapSelectedBook || {};
        ...
        availableBooks.forEach((book, index) => {
            ...
            const pct = window.ReadingState.bookProgress[book.id] || 0;
            ...
            btnBg.on('pointerdown', () => {
                ...
                // 未完成书 → 原逻辑
                const prevBookId = window.ReadingState.mapSelectedBook[mapKey] || null;

                if (!prevBookId) {
                    window.ReadingState.mapSelectedBook[mapKey] = book.id;
                } else if (prevBookId !== book.id) {
                    window.ReadingState[storageKey] = 0;
    ...
    updateTokenPosition(animate = true) {
        ...
        if (targetIndex === this.pointPositions.length - 1) {
            ...
            if (!window.ReadingState._continentCompletedFlags) {
                window.ReadingState._continentCompletedFlags = {};
            }

            const mapKey = this.scene.key;

            // 只提示一次
            if (!window.ReadingState._continentCompletedFlags[mapKey]) {
                window.ReadingState._continentCompletedFlags[mapKey] = true;
