class ReadingScene extends Phaser.Scene {
    ...
    create() {
        ...
        // 恢复阅读进度（按书 ID）
        const savedPct = window.ReadingState.bookProgress[this.bookData.id] || 0;
        ...
        window.ReadingState.progress = savedPct;
        ...
        const updateScroll = (deltaY) => {
            ...
            window.ReadingState.progress = pct;
        };
        ...
        closeBtn.on('pointerdown', () => {
            const finalPct = window.ReadingState.progress || 0;

            // 保存阅读进度（按书 ID）
            window.ReadingState.bookProgress[this.bookData.id] = finalPct;

            // ⭐⭐⭐ 只读模式：不更新地图、不解锁洲、不动 token ⭐⭐⭐
            if (this.readOnly) {
            ...
            // ⭐⭐⭐ 正常模式：更新地图进度 ⭐⭐⭐
            if (this.sourceMap) {
                const cfg = window.ReadingState.mapConfig[this.sourceMap];

                if (cfg) {
                    const storageKey = cfg.storage;

                    // 洲进度 = 当前书进度（驱动 token）
                    window.ReadingState[storageKey] = finalPct;

                    // 如果书读完（100%）
                    if (finalPct >= 100) {
                        // 标记书已完成
                        if (!window.ReadingState.completedBookIds[this.bookData.id]) {
                            window.ReadingState.completedBookIds[this.bookData.id] = true;
                            window.ReadingState.booksRead += 1;
                        }

                        // 解锁下一个洲
                        const order = window.ReadingState.mapOrder || [];
                        const idx = order.indexOf(this.sourceMap);
                        if (idx !== -1 && idx < order.length - 1) {
                            const nextMap = order[idx + 1];
                            if (window.ReadingState.mapUnlock && nextMap) {
                                window.ReadingState.mapUnlock[nextMap] = true;
                            }
                        }
                    }
                }
